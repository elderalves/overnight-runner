...
/** The gaps of a hunk list. `includeTrailing` is always false in this port -- see
 *  `contextLinesForGap`'s note: without a `loadFileText` source there is nothing to expand
 *  a gap into, so gaps always render as static separators (types.ts). */
...
export type UnifiedRow = { type: 'hunk'; hunk: Hunk } | { type: 'gap'; gap: ContextGap } | { type: 'line'; cell: DiffCell };

export type SplitRow = { type: 'hunk'; hunk: Hunk } | { type: 'gap'; gap: ContextGap } | { type: 'pair'; left?: DiffCell; right?: DiffCell };
...
export function buildUnifiedRows(hunks: Hunk[], gaps: ContextGap[] = []): UnifiedRow[] {
...
export function buildSplitRows(hunks: Hunk[], gaps: ContextGap[] = []): SplitRow[] {
  const gapBefore = new Map(gaps.map((gap) => [gap.beforeHunk, gap]));
  const rows: SplitRow[] = [];
  const pushGap = (position: number) => {
    const gap = gapBefore.get(position);
