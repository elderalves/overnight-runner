...
/**
 * Word spans for one del/add line pair, or null when marks would not help the
 * reader (identical lines, rewrites, degenerate or oversized input).
 */
export function diffWords(before: string, after: string): WordDiff | null {
  ...
  // Classic LCS table; dimensions are capped above so worst case is 300x300 numbers.
  const width = b.length + 1;
  const dp = new Uint16Array((a.length + 1) * width);
  for (let i = 1; i <= a.length; i++) {
    for (let j = 1; j <= b.length; j++) {
      dp[i * width + j] =
        a[i - 1] === b[j - 1] ? (dp[(i - 1) * width + (j - 1)] ?? 0) + 1 : Math.max(dp[(i - 1) * width + j] ?? 0, dp[i * width + (j - 1)] ?? 0);
    }
  }
...
/**
 * Merge a line's syntax tokens with its word spans into flat segments,
 * splitting at every boundary of either sequence. Both sequences concatenate
 * to the same line text; if tokens are missing (grammar not yet loaded) the
 * spans alone render, and vice versa.
 */
export function overlaySegments(tokens: readonly SynToken[] | null, spans: readonly WordSpan[] | null | undefined, text: string): RenderSegment[] {
