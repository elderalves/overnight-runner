...
import { useGitCommit } from '@/api/queries';
...
export function RepoCommitsSection({ log }: { log: LogEntry[] }) {
  ...
  if (log.length === 0) {
    return <CenteredState icon={<GitCommitHorizontalIcon />} tone="neutral" heading="h2" title="No commits yet" subtitle="The log is empty — this repository has no commits to show." />;
  }
  return (
    <CommitList
      slot="repo-commits"
